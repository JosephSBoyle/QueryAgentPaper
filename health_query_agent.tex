...
% QA 				& \SR\% & -5 & -1,500,000 \\
% \hline
% QA + sub-agent			& \SRRetrieval\% & -5 & -1,000,000 \\
% \hline
% QA + sub-agent w/ UMLS		& \SRRetrievalUMLS\% & -6 & -1,000,000 \\
% \hline
% \end{tabular}
% 	\caption{Performance for QueryAgent (QA) with ablations for it's Retrieval sub-agent and the UMLS knowledge base on the development set.}
...
\hline
    & \multicolumn{1}{c}{Accuracy} & \multicolumn{1}{c}{Query Type} & \multicolumn{3}{c}{Patient Overlap} & Mean tokens \\
% \hline
    \cmidrule(lr){2-2} \cmidrule(lr){3-3} \cmidrule(lr){4-6} \cmidrule(lr){7-7}

Single Questions &  (\%) & (\%) & rec. & prec. & F1-score & \\ 
\hline
\textsc{QueryAgent} & \Accuracy{} (\Passed{} / \Total{}) & \textbf{\Semanticparsingjaccardscore} & \textbf{\Recall} & \Precision & \Fscore & \Tokensmean \\

\textsc{+ RetrievalAgent} & \irAccuracy{} (\irPassed{} / \irTotal{}) & \textbf{\irSemanticparsingjaccardscore} & \irRecall & \textbf{\irPrecision} & \irFscore & \irTokensmean \\

\textsc{+ RetrievalAgent} + UMLS & \textbf{\IRumlsAccuracy{} (\IRumlsPassed{} / \IRumlsTotal{})} &  \IRumlsSemanticparsingjaccardscore & \IRumlsRecall & \IRumlsPrecision & \IRumlsFscore & \textbf{\IRumlsTokensmean} \\
\hline
\multicolumn{7}{l}{Chained Questions} \\
\hline
\textsc{QueryAgent} & \sAccuracy{} (\sPassed{} / \sTotal{}) & \sSemanticparsingjaccardscore & \sRecall & \sPrecision & \sFscore & \sTokensmean \\
\textsc{+RetrievalAgent} &  \sirAccuracy{} (\sirPassed{} / \sirTotal{}) & \sirSemanticparsingjaccardscore & \sirRecall & \sirPrecision & \sirFscore & \sirTokensmean \\
\hline
\end{tabular}
}
\caption{
