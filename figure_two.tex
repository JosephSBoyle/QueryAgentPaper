...
% --- NODES ---
% Central agent nodes
\node[box-green] (ehealth) {};
\node at ([yshift=1.2cm]ehealth.center) {eHealthScope};
\node[box-purple] at (ehealth.center) (agent) {HealthQuery\\Agent};

% Databases at the bottom
\node[database, below left=of ehealth, xshift=-1cm, yshift=-1.5cm] (patient_data)   {Patient data};
\node[database, below=of agent, yshift=-2cm]                       (concept_db)     {Concept database};
...
% web resources
\node[box-blue, right=of ehealth, yshift=2.5cm, xshift=2.5cm]   (llm) {LLM\\ Gemini; \\ ChatGPT; \\ Claude};
\node[database, below=of llm, xshift=1cm, yshift=-1cm]          (umls) {UMLS};

% NHS Intranet
\node[nhs-cloud, fit=(shc_icon) (ssc_icon) (phc_icon) (ehealth) (patient_data) (concept_db), label={[red, font=\bfseries]above:NHS intranet}] (nhs_fit) {};
% --- CLOUDS (fitted around other nodes) ---
...
% --- LINES & ARROWS ---
% Dotted line
\draw[dashed, thick, red] (ehealth.north west) -- (ehealth.south west |- patient_data.south);
...
\draw[arrow] (ehealth.west) to[bend left=10] (patient_data.north);
\draw[arrow] (agent) -- (umls);
\draw[arrow] (agent) -- (concept_db);

\draw[arrow] (agent.east) to[out=0, in=180] (llm.west);
