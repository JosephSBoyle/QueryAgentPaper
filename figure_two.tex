...
\node[database, below left=of ehealth, xshift=-2cm, yshift=-2.7cm] (patient_data)   {Patient Data};
\node[database, below right=of agent, yshift=-2cm]               (concept_db)       {Concepts};

% Primary Care - Circle with P (Green - least severe/first contact)
\node[draw=green!70!black, fill=green!20, thick, circle, minimum size=2cm,
      left=of patient_data, yshift=6cm, xshift=-3cm] (phc_icon) {Outpatient};

% Secondary Care - Circle with H (Orange - moderate severity)
\node[draw=orange!70!black, fill=orange!20, thick, circle, minimum size=2cm,
      below left=of phc_icon] (shc_icon) {Inpatient};

% Tertiary Care - Circle with S (Red - highest severity/specialized)
\node[draw=red!70!black, fill=red!20, thick, circle, minimum size=2cm,
      below right=of shc_icon] (ssc_icon) {Specialist};
...
\node[ehs, fit=(agent) (ehealth) (patient_data) (concept_db),
    label={[color={rgb,255:red,0;green,94;blue,184}, font=\bfseries]\Large eHealthScope}] (ehealth) {};
...
\draw[arrow] (patient_data) to[bend right=10]  (ehealth.west);
