...
% 	By looking up `ddC', the agent is able to link it to the brand-name Zalcatabine, . % a process which involves multi-step information retrieval.
...
\begin{tabular}{lcccr}
\hline
    & \multicolumn{3}{c}{Patient Overlap} & Mean tokens \\
    \cmidrule(lr){2-4}
Single Questions & Recall & Precision & F1-score & consumed \\ 
\hline
\textsc{QueryAgent} & \Recall & \Precision & \Fscore & \Tokensmean \\

\textsc{+ RetrievalAgent} &  \irRecall & \irPrecision & \irFscore & \irTokensmean \\
...
\textsc{+RetrievalAgent} & \sirRecall & \sirPrecision & \sirFscore & \sirTokensmean \\
\textsc{+RetrievalAgent} + UMLS  & \sirumlsRecall & \sirumlsPrecision & \sirumlsFscore & \sirumlsTokensmean \\
